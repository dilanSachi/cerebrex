...
class HiruTamilBot(scrapy.Spider):
    name = "HiruTamilBot"
    ...
    def writeToJson(self, header, time, content, docId, url):
        ...
        Path("../../../data/hirunews/bot/tamil").mkdir(parents=True, exist_ok=True)
        with open("../../../data/hirunews/bot/tamil/" + docId + ".json", 'a', encoding="utf8") as ofile:
    ...
    def parse(self, response):
        ...
        if (len(newsLinks) > 0):
            ...
            if("local-news" in url):
                self.oldLocalLink = crawled["hirutamillocal"]
                crawled["hirutamillocal"] = firstlink
            elif("international-news" in url):
                self.oldInternationalLink = crawled["hirutamilinternational"]
                crawled["hirutamilinternational"] = firstlink
            else:
