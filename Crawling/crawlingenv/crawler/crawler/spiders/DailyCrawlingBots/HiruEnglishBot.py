...
class HiruEnglishBot(scrapy.Spider):
    name = "HiruEnglishBot"
    ...
    def writeToJson(self, header, time, content, docId, url):
        ...
        Path("../../../data/hirunews/bot/english").mkdir(parents=True, exist_ok=True)
        with open("../../../data/hirunews/bot/english/" + docId + ".json", 'a', encoding="utf8") as ofile:
    ...
    def parse(self, response):
        ...
        if (len(newsLinks) > 0):
            ...
            if("local-news" in url):
                self.oldLocalLink = crawled["hiruenglishlocal"]
                crawled["hiruenglishlocal"] = firstlink
            elif("international-news" in url):
                self.oldInternationalLink = crawled["hiruenglishinternational"]
                crawled["hiruenglishinternational"] = firstlink
            else:
